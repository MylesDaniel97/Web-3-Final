...
var resultEl = document.querySelector(".resultSet"),
  plusMinusWidgets = document.querySelectorAll(".qty-input");

// Attach the handlers to each plus-minus thing
for (var i = 0; i < plusMinusWidgets.length; i++) {
  plusMinusWidgets[i].querySelector(".qty-count--minus").addEventListener("click", clickHandler);
  plusMinusWidgets[i].querySelector(".qty-count--add").addEventListener("click", clickHandler);
  plusMinusWidgets[i].querySelector(".product-qty").addEventListener("change", changeHandler);
}
...
function clickHandler(event) {
  ...
  var countEl = event.target.parentNode.querySelector(".product-qty");
  if (event.target.className.match(/\bqty-count--minus\b/)) {
    countEl.value = Number(countEl.value) - 1;
  } else if (event.target.className.match(/\bqty-count--add\b/)) {
    countEl.value = Number(countEl.value) + 1;
...
function changeHandler(event) {
  ...
  for (var i = 0; i < plusMinusWidgets.length; i++) {
    ...
    resultEl.value = Number(resultEl.value) + Number(plusMinusWidgets[i].querySelector('.product-qty').value);
